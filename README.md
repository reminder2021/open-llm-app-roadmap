...
- 没有编程基础但想学习 LLM 应用开发的测试人员、产品经理（建议先学 Python 零基础入门）
...
| 阶段 | 内容 | 新增内容 |
| --- | --- | --- |
| 前置 | Git 入门 | ✅ Git 基础教程（5 章，专为不会 Git 的学习者设计） |
| 00 | 总览与学习方式 | |
| 01 | Python 与 Web API 基础 | ✅ Python 零基础入门（8 章 + 4 个示例大全，专为无编程基础的学习者设计） |
| 02 | 开源模型使用与推理服务 | ✅ 中文模型实践经验（Qwen、ChatGLM、Yi、DeepSeek） |
| 03 | Prompt 与结构化输出 | ✅ Prompt 评估（提前引入测试集概念） |
| 04 | RAG 知识库问答 | |
| 05 | 工具调用、LangChain 与 LangGraph | |
| 06 | LoRA/QLoRA 微调 | |
| 07 | 评估体系 | |
| 08 | 部署与工程化 | ✅ 成本控制与 Token 计费 |
| 09 | 综合项目作品集 | ✅ 前端展示方案（Gradio、Streamlit、React、HTML） |

## 推荐学习顺序

```text
[可选] Git 入门（不会 Git 的学习者）
→ [可选] Python 零基础入门（没有编程基础的学习者）
→ Python / Web API 基础
→ 开源模型推理
→ Prompt 与结构化输出
→ RAG
